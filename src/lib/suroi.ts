import { ObjectCategory } from 'suroi/common/src/constants';
import { type BuildingDefinition, Buildings } from 'suroi/common/src/definitions/buildings';
import { type DecalDefinition, Decals } from 'suroi/common/src/definitions/decals';
import type { GunDefinition } from 'suroi/common/src/definitions/guns';
import {
	type LootDefinition,
	Loots,
	type WeaponDefinition
} from 'suroi/common/src/definitions/loots';
import { type ObstacleDefinition, Obstacles } from 'suroi/common/src/definitions/obstacles';
import {
	type ItemDefinition,
	ItemType,
...
export function getSuroiImageLink<T extends ObjectDefinition | ItemDefinition>(
	...
	// Is a building?
	if (isBuilding(obj)) return _otherImageLink(obj, ObjectCategory.Building, variation);

	// Is an obstacle?
	if (isObstacle(obj)) return _otherImageLink(obj, ObjectCategory.Obstacle, variation);

	// Is a decal?
	if (isDecal(obj)) return _otherImageLink(obj, ObjectCategory.Decal, variation);
...
function _itemImageLink(
	...
	return `${IMAGE_BASE_URL}${IMAGE_BASE_URLS[ItemType[itemType] as keyof typeof ItemType]}/${
		dual ? idString : idString.replace('dual_', '')
	}${variation ? `_${variation}` : ''}${
		append ? (Array.isArray(append) ? '_' + append.join('_') : `_${append}`) : ''
	}.svg`;
}
...
function _otherImageLink<Category extends ObjectCategory>(
	...
	return `${IMAGE_BASE_URL}${IMAGE_BASE_URLS[key]}/${
		isBuilding(obj) ? obj.ceilingImages?.[0].key || obj.floorImages?.[0].key : obj.idString
...
function isBuilding(obj: ObjectDefinition): obj is BuildingDefinition {
	return Boolean(Buildings.definitions.find((building) => building.idString === obj.idString));
}

function isObstacle(obj: ObjectDefinition): obj is ObstacleDefinition {
	return Boolean(Obstacles.definitions.find((obstacle) => obstacle.idString === obj.idString));
}

function isDecal(obj: ObjectDefinition): obj is DecalDefinition {
	return Boolean(Decals.definitions.find((decal) => decal.idString === obj.idString));
}

function isLoot(obj: ObjectDefinition): obj is LootDefinition {
	return Boolean(Loots.definitions.find((loot) => loot.idString === obj.idString));
}
